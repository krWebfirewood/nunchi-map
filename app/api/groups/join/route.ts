...
export async function POST(request: Request) {
  ...
  const group = await db.group.findUnique({ where: { inviteCode: parsed.data.inviteCode }, select: { id: true, name: true, inviteCode: true, _count: { select: { members: true } } } });
  ...
  return Response.json({ group: { id: group.id, name: group.name, inviteCode: group.inviteCode, memberCount } });
}
