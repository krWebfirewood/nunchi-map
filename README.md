...
그룹 카드의 `현재 위치 공유`를 누르면 브라우저 GPS 위치를 해당 그룹에만 공유합니다. 다른 그룹 구성원은 `지도에서 보기`로 최근 위치를 확인할 수 있으며, 이름과 GPS 정확도 범위가 지도에 표시됩니다. 공유 데이터는 45초마다 갱신되고 마지막 갱신 후 2분이 지나면 자동으로 숨겨집니다. `위치 공유 중지`를 누르면 즉시 삭제되며, 페이지를 닫거나 모바일 브라우저가 중단되면 만료 시간 이후 자동으로 숨겨집니다. 위치 권한은 기본적으로 꺼져 있고 사용자가 버튼을 누른 경우에만 요청합니다.
