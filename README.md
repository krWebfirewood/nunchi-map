...
첫 화면에서 새 계정을 만들거나 기존 계정으로 로그인할 수 있으며, 개발용 데모 사용자를 바로 선택할 수도 있습니다. 비밀번호는 평문으로 저장하지 않고 Node.js `scrypt`로 해시합니다. 로그인하면 7일짜리 HttpOnly 로컬 세션이 생성됩니다. 이후 일정 조회·등록·삭제 API는 브라우저가 보낸 세션을 기준으로 사용자를 확인하며, 클라이언트가 임의로 보낸 사용자 ID를 신뢰하지 않습니다. 그룹 충돌 검사는 본인 일정을 개수에서 제외하고, 현재 사용자와 하나 이상의 비공개 그룹을 공유하는 다른 구성원의 공유 일정만 대상으로 시간과 위치를 함께 비교합니다. 일정별 `그룹 공유` 설정은 기본으로 켜져 있으며 등록 전이나 저장 후에 `나만 보기`로 바꿀 수 있습니다. 공유 일정의 위치와 시간은 같은 비공개 그룹 지도에 표시하되 사용자 이름은 숨깁니다. 내 일정 또는 그룹 일정과 시간이 겹쳐도 위험 경고와 배지를 표시한 뒤 저장할 수 있습니다. 기본 데모 그룹의 초대 코드는 `NUNCHI`입니다.
...
그룹 카드의 `현재 위치 공유`를 누르면 브라우저 GPS 위치를 해당 그룹에만 공유합니다. 다른 그룹 구성원은 `지도에서 보기`로 최근 위치를 확인할 수 있으며, 이름과 GPS 정확도 범위가 지도에 표시됩니다. 오차가 500m보다 큰 위치는 바로 공유하지 않고 브라우저에서 더 정확한 GPS 좌표를 기다립니다. 공유 데이터는 45초마다 갱신되고 마지막 갱신 후 2분이 지나면 자동으로 숨겨집니다. `위치 공유 중지`를 누르면 즉시 삭제되며, 페이지를 닫거나 모바일 브라우저가 중단되면 만료 시간 이후 자동으로 숨겨집니다. 위치 권한은 기본적으로 꺼져 있고 사용자가 버튼을 누른 경우에만 요청합니다.
